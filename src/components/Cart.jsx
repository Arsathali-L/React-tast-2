...
const Cart = ({ cartValue, removefun }) => {
  ...
  return (
    <article className="bg-pink-200 h-full max-lg:h-full max-md:w-full  pt-4 pb-10">
      <Link to="/">
        <div>
      ...
      {cartValue?.length === 0 ? (
        <div className="text-3xl font-bold text-center pt-10 max-lg:text-xl h-screen bg-orange-500">
          Cart is Empty
        </div>
      ) : (
        cartValue.map((item) => (
          <div className="cart-item " key={item.id}>
            <div className="flex justify-center items-center max-md:gap-5 gap-20 max-lg:gap-10 pb-5 border-2 border-blue-950 mx-7 bg-blue-300 pt-5 max-md:border-none max-md:flex max-md:flex-col">
              <img
                src={item.image}
                alt={item.title}
              ...
              <div
                className="p-4 bg-red-600 text-white rounded-md cursor-pointer max-md:p-2"
      ...
      {cartValue.length == 0 ? null : (
        <>
          ...
          <div className="flex justify-center items-center gap-10 pb-10  mx-6">
            <div className="text-4xl p-4 rounded-xl font-bold text-purple-900 max-md:text-3xl">
              Discount Value :
            </div>
            <div className="text-3xl font-bold text-green-700  ">${discount}</div>
          </div>
        </>
      )}
    </article>
